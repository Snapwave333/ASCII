# Build, Run, and Verify the Vulkan ASCII VJ Application
...
### Color Channel Swap (Red/Blue Inverted)

- Symptoms: colors appear incorrect, red shows as blue, screen looks blue-tinted.
- Root cause: pixel byte order mismatch between CPU composition and swapchain format.
- Verify:
  - Swapchain format is `VK_FORMAT_B8G8R8A8_UNORM` in `VulkanContext::ChooseSwapSurfaceFormat`.
  - `Renderer::ComposeFramePixels` packs bytes as BGRA:
    - `p[0] = rgba & 0xFFu`
    - `p[1] = (rgba >> 8) & 0xFFu`
    - `p[2] = (rgba >> 16) & 0xFFu`
    - `p[3] = (rgba >> 24) & 0xFFu`
- Solution: ensure BGRA packing throughout CPU→staging→image pipeline.

### Visual Verification Test Card

- Purpose: quickly validate channel correctness and canvas rendering.
- Usage:
  - Call `Renderer::RenderTestCard("auto")` early during startup.
  - Confirm primary/secondary/accent panels display expected red/green/blue.
  - Verify grid and bars render with correct positions and colors.
